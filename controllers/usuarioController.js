...
const cargosValidos = ['diretor', 'diretor_adjunto', 'secretaria', 'professor', 'outros'];

const permissoesPorCargo = {
  diretor: ['cadastro', 'documentos', 'termos', 'declaracoes', 'matricula', 'transferencia', 'rematricula', 'relatorios', 'aluno'],
  diretor_adjunto: ['cadastro', 'documentos', 'termos', 'declaracoes', 'matricula', 'transferencia', 'rematricula', 'relatorios', 'aluno'],
  secretaria: ['documentos', 'relatorios', 'matricula', 'transferencia', 'aluno', 'rematricula'],
...
exports.cadastrarUsuario = async (req, res) => {
  ...
  if (!req.usuario || (req.usuario.cargo !== 'diretor' && req.usuario.cargo !== 'diretor_adjunto')) {
    return res.status(403).json({ mensagem: 'Apenas diretores ou diretores adjuntos podem cadastrar usuários.' });
  }

  // Validação de campos obrigatórios
  if (!usuario || !senha || !email || !cargo) {
    return res.status(400).json({ mensagem: 'Preencha usuario, senha, email e cargo!' });
  }
  ...
  try {
    ...
    const [usuarioExistente] = await db.query('SELECT * FROM usuarios WHERE nome = ?', [usuario]);
    if (usuarioExistente.length > 0) {
      return res.status(400).json({ mensagem: 'Usuário já existe!' });
    }

    const [emailExistente] = await db.query('SELECT * FROM usuarios WHERE email = ?', [email]);
...
await db.query(
  ...
  const corpoHtml = `
    ...
    //retorna a resposta
    res.json({ mensagem: 'Verifique seu e-mail. Enviamos um link para redefinir sua senha.' });
  } catch (err) {
...
const [verifica] = await db.query(
  ...
  [token]
    ...
    if (result.affectedRows === 0) {
      ...
      return res.status(500).json({ mensagem: 'Não foi possível atualizar a senha.' });
    }
...
exports.editarPerfil = async (req, res) => {
  ...
  try {
    ...
    await db.query(
      ...
      [nome || rows[0].nome, email || rows[0].email, id]
    );
...
exports.definirPermissoesUsuario = async (req, res) => {
  ...
  if (!Array.isArray(permissoes) || permissoes.length === 0) {
    return res.status(400).json({ mensagem: 'Permissões devem ser um array não vazio.' });
  }
